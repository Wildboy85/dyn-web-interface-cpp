...
// windows 10 install all sdk for windows.h 
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	//std::wstring project01 = L"copyhyperlink";
	...
	if (project01 == L"copyhyperlink")
	{
		// buttons specific to one project
		arrbutnam.push_back(L"copyhyperlink");
		arrbutdes.push_back(L"CopyHyperLink");
		arrdepnam.push_back(L"util");
		arrdepcol.push_back(L"cccccc");
...
std::vector<std::wstring> dynforgen(IHTMLDocument2* fra, std::wstring *title01, 
	...
	std::vector<std::wstring>* buttmp)
{
	// generate html textbox input, ok and cancel
